...
//Add extensions to the new DB
async function addExtensions(){
    connectToDB("pgClient");
    return pgClient.query(`CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`)
        .then(() => console.log("Added extension uuid-ossp.."))
        .catch(err => console.log("Failed to ddded extension uuid-ossp\n"+err.stack));
}
...
async function checkIfAppDBAndUserExists() {
    ...
    //Add extensions
    await addExtensions();
    await disconnectFromDB('pgClient');
}
